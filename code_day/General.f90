...
! VALID
      REAL GEP_V,ET_V,GPP_V,NPP_V,FLOW_V,FLOW,RUNOFF_V,RUN_OFF,BASEFLOW_V,&
        BASEFLOW
      COMMON/VALID/ GEP_V(50000,32,12), ET_V(50000,32,12)&
          ,GPP_V(50000,32),NPP_V(50000,32), FLOW_V(32,12),FLOW(32,12)&
          ,RUNOFF_V(50000,32),RUN_OFF(50000,32), &
          BASEFLOW_V(32,12),BASEFLOW(50000,32,12)

! CELLINFO
      REAL LADUSE,LATUDE,LONGI,HUCELE
      INTEGER HUCNO
      COMMON/CELLINFO/LADUSE(50000),HUCNO(50000),LATUDE(50000),LONGI(50000),HUCELE(50000)

! VEGINFO
      INTEGER veg
      COMMON/VEGINFO/VEG(50000,32)
...
! OUTPUT1
      ...
      COMMON/OUTPUT1/ PET(32,12),APET(12),PAET(32,12),APAET(12),&
         ...
         A_ET(50000,32, 12),P_ET(50000,32,12),Sun_ET(50000,32,12),&
         RUN_HRU(50000,32,12),BASE_HRU(50000,32,12)  

! Monthly RUNOFF

       Real RUNOFF_MON, BASEFLOW_MON
       COMMON/Month_RUN/ RUNOFF_MON(50000,12),BASEFLOW_MON(50000,12)
      
! CLIMATE      
      REAL  RAIN,TEMP,AAPPT
      COMMON/CLIMATE/ RAIN(50000,32,12), TEMP(50000,32,12), AAPPT(50000)
      
! LAI        
      REAL LAI
      COMMON/LAI/LAI(50000,32,12)
...
! SOIL
      REAL LZTWM, LZFPM, LZFSM,LZSK,LZPK, UZTWM, UZFWM, UZK, ZPERC,&
        REXP, PFREE, SMC        
      COMMON/SOIL/LZTWM(50000), LZFPM(50000), LZFSM(50000), LZSK(50000),&
        LZPK(50000), UZTWM(50000), UZFWM(50000), UZK(50000), ZPERC(50000),&
        REXP(50000), PFREE(50000), SMC(12)
...
! FLOW 
      ...
      COMMON/FLOW/STRFLOW(50000,32,12),STRET(50000,32,12)&
     ,STRGEP(50000,32,12)

! CARBON
        REAL GEPM,RECOM,NEEM,GEPA,NEEA
      COMMON/CARBON/ GEPM(50000,32,12),RECOM(50000,32,12), &
      NEEM(50000,32,12),GEPA(50000,32),NEEA(50000,32)
      
! BYLAND   
      REAL,ALLOCATABLE :: RUNLAND(:,:,:,:),ETLAND(:,:,:,:),GEPLAND(:,:,:,:)
!	   COMMON/BYLAND/RUNLAND,ETLAND,GEPLAND
!      REAL RUNLAND,ETLAND,GEPLAND    
!      COMMON/BYLAND/ RUNLAND(50000,32,12,31), &
!        ETLAND(50000,32,12,31), GEPLAND(50000,32,12,31)

! HUCPETAET
      REAL HUCAET, HUCPET,HUCPAET
      COMMON/HUCPETAET/HUCAET(50000,32), HUCPET(50000,32),&
      HUCPAET(50000,32)
...
!--Open Input files----------------------------------------------
	     ...
	     input_mulu="F:\Data_WaSSIC\WaSSI_ZA\inputs_82_12"
	     output_mulu='F:\Data_WaSSIC\WaSSI_ZA\outputs_82_12'
