...
!！！！！！！！！更改591行的碳平衡计算公式                             C
!！！！！！！！817行RUNLAND (流域数，年数，月份，月份，植被类型数）数组大小C
!**********************************************************************C
...
! -- CROP
               ...
               GEP(J, M, K) = 4.5 * ET(J,M,K)
!---------论文中的公式
!              RECO(J,M,K)= 40.6 + 1.35 * ET(J,M,K)*MNDAY
!---------原始计算公式 		
               RECO(J,M,K)= 11.4 + 1.85 * GEP(J,M,K)*MNDAY      !11.14 1.85
...
!-- DECIDUOUS Broadleaf FOREST
               ...
               GEP(J, M, K) = 2.4* ET(J,M,K) 
!-------论文公式				   
!               RECO(J,M,K)= 30.8 + 1.44 * ET(J,M,K)*MNDAY	
!-------原始计算公式
			   RECO(J,M,K)= 24.12 + 1.49 * ET(J,M,K)*MNDAY	
...
! -- GRASSLANDS               
               ...
               GEP(J, M, K) = 2.25 * ET(J,M,K)
!------ 论文公式
!               RECO(J,M,K)= 18.9 + 1.36* ET(J,M,K)*MNDAY
!-------原始计算公式	
			   RECO(J,M,K)= 14.2 + 1.42 * ET(J,M,K)*MNDAY	
