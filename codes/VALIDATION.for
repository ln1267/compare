...
C        I=HUC; J= YEAR, M =MONTH, MNDAY= NO OF DAYS IN A MONTH
     ...
     >,A_ET(1000,200, 12),P_ET(1000,200, 12),SP_ET(1000,200, 12)
...
1003  CONTINUE 
            ...
            BASEFLOW(J,M)=BASEFLOW_L/UNIONAREA*100000    
1002  CONTINUE
1001  CONTINUE
...
C---------计算BASEFLOW的皮尔森相关系数（P)，决定系数（R），效率系数（NS）
        num_F=0
            DO 13002 J=1,15
                DO 13003 M=1,12

                num_F=num_F+1
                SUM_BASEFLOW=SUM_BASEFLOW+BASEFLOW(J,M)
                SUM_BASEFLOW_V=SUM_BASEFLOW_V+BASEFLOW_V(J,M)
                                
                SUM_BASEFLOW_X=SUM_BASEFLOW_X+BASEFLOW(J,M)
                SUM_BASEFLOW_Y=SUM_BASEFLOW_Y+BASEFLOW_V(J,M)
                SUM_BASEFLOW_XY=SUM_BASEFLOW_XY+BASEFLOW(J,M)*
     >BASEFLOW_V(J,M)
                SUM_BASEFLOW_XX=SUM_BASEFLOW_XX+BASEFLOW(J,M)**2
...
13002   Continue
      ...
      SUM_BASEFLOW_U=SUM_BASEFLOW_U+(BASEFLOW(J,M)-AVE_BASEFLOW)*
     >(BASEFLOW_V(J,M)-AVE_BASEFLOW_V)
      SUM_BASEFLOW_L=SUM_BASEFLOW_L+(BASEFLOW_V(J,M)-AVE_BASEFLOW_V)**2
      SUM_BASEFLOW_R=SUM_BASEFLOW_R+(BASEFLOW(J,M)-AVE_BASEFLOW)**2
      SUM_BASEFLOW_N=SUM_BASEFLOW_N+(BASEFLOW(J,M)-BASEFLOW_V(J,M))**2
...
!     >           ',', E15.6)   
        ...
        WRITE(2003,20003)VAL_1(TUN1), R_RUNOFF,R_BASEFLOW,
     > NS_RUNOFF,NS_BASEFLOW,P_RUNOFF,P_BASEFLOW

        WRITE(*,20003)VAL_1(TUN1), R_RUNOFF,R_BASEFLOW,
     > NS_RUNOFF,NS_BASEFLOW,P_RUNOFF,P_BASEFLOW
