...
! ----------------------------------------------------------------------------     
      ...
      REAL TAUZTWC, TAUZFWC, TALZTWC, TALZFPC, TALZFSC,TAREA
...
50          CONTINUE
        ...
        ELSEIF (J .GT. 1 .AND. M .EQ. 1) THEN

             DO 60 K=1, NLC
         
               UZTWC(K) = MonUZTWC(I,J-1,12)
               UZFWC(K) = MonUZFWC(I,J-1,12)
               LZTWC(K) = MonLZTWC(I,J-1,12)
               LZFSC(K) = MonLZFSC(I,J-1,12)
               LZFPC(K) = MonLZFPC(I,J-1,12)
...
60          CONTINUE
        ...
        ELSE
            DO 70 K=1, NLC
               ...
               UZTWC(K) = MonUZTWC(I,J,M-1)
               UZFWC(K) = MonUZFWC(I,J,M-1)
               LZTWC(K) = MonLZTWC(I,J,M-1)
               LZFSC(K) = MonLZFSC(I,J,M-1)
               LZFPC(K) = MonLZFPC(I,J,M-1)
...
!print*, UZTWC(K),UZFWC(K),LZTWC(K),LZFSC(K),LZFPC(K)
! *****************************************************************************************************
...
! for check
!WRITE(*,*) I,J,M,K,' ET=', ET(J,M,K),'TEMP=',TEMP (I,J, M),'UZTWC=',UZTWC(K),'PBF=',PBF(K),'SBF=',SBF(K),'LZFPC=',LZFPC(K) ,&
!'LZFSC=',LZFSC(K),'UZFWC=', UZFWC(K),'SURFRO=', SURFRO(K),'INF=', INF(K)
...
!---------论文中的公式
               RECO(J,M,K)= 40.6 + 0.43 * GEP(J, M, K)
!---------原始计算公式 		
...
! -- Close SHRUBLANDS                   
               ...
               RECO(J,M,K)= 11.4 + 0.69 * GEP(J, M, K)
...
!------论文公式				   
               RECO(J,M,K)= 30.8 + 0.45 * GEP(J, M, K)	
!-------原始计算公式
...
! -- Evergreen Broadleaf FOREST
               ...
               RECO(J,M,K)= 19.6 + 0.61 * GEP(J, M, K)		   

!--- Evergreen Needleleaf Forest
               ELSEIF (K .EQ. 3) THEN                

               GEP(J, M, K) = 2.14* ET(J,M,K)
               RECO(J,M,K)= 9.9 + 0.68 * GEP(J, M, K)	   
...
!------ 论文公式
              RECO(J,M,K)= 18.9 + 0.64*GEP(J, M, K)
!-------原始计算公式	
...
!---- MIXED FOREST
               ...
               RECO(J,M,K)= 24.44 + 0.62 * GEP(J,M,K)
...
!-- SAVANNAS                   
	     ...
	     ELSEIF (K.EQ.  0) THEN                
               ...
               RECO(J,M,K)= 25.2 + 0.53 * GEP(J, M, K)
...
! -- Wetlands                     
	     ...
	     ELSEIF (K .EQ. 0) THEN                
               ...
               RECO(J,M,K)= 7.8 + 0.56 * GEP(J, M, K)    
!-- Wet Savanna                     
               
	     ELSEIF (K .EQ. 0) THEN                

               GEP(J, M, K) = 1.49* ET(J,M,K)
               RECO(J,M,K)= 14.7 + 0.63 * GEP(J, M, K)
...
!			RUNLAND_Y=(J-1)*12+M							
            RUNLAND(I,J,M, K) = SURFRO(K) + PBF(K) + SBF(K) + INF(K)
            ETLAND(I,J,M, K) = ET(J,M,K)
            GEPLAND(I,J,M, K) = GEP(J,M,K)
            NEELAND(I,J,M, K) = NEE(J,M,K)
...
!100        CONTINUE
           ...
           MonUZTWC(I,J,M) = AUZTWC
           MonUZFWC(I,J,M) = AUZFWC
           MonLZTWC(I,J,M) = ALZTWC
           MonLZFPC(I,J,M) = ALZFPC
           MonLZFSC(I,J,M) = ALZFSC
...
! -- STREAMFLOW IN MILLION M3 FOR EACH HUC FOR MONTH M. HUCAREA IN SQ. METERS 
          STRFLOW(I, J, M) = (RUNOFF(M) + PRIBF(M) + SECBF(M) + INTF(M)) * HUCAREA(I)/1000./1000000.

! TEST OUTPUT

    WRITE(99,*) 'ICELL=',I,'Year=',J,'Month=',M,'TEMP=',TEMP(I,J,M),'RAINFALL=',RAIN(I,J,M),'AET=',AET(M),&
    'SMC=',SMC (M),'SP=',SP(M), 'RUNOFF=',RUNOFF(M),'PRIBF=',PRIBF(M),'SECBF=',SECBF(M),'INTF=',INTF(M),&
        'AVUZTWC=',AVUZTWC(M),'AVUZFWC=',AVUZFWC(M),'AVLZFPC=',AVLZFPC(M),'AVLZFSC=',AVLZFSC(M),&
        'GEPM=',GEPM(I,J, M) 
12133 format(A10,I6,I6,I4,15F10.3)
