...
! VALID
      REAL GEP_V,ET_V,GPP_V,NPP_V,FLOW_V,FLOW,RUNOFF_V,RUN_OFF,BASEFLOW_V,&
        BASEFLOW
      COMMON/VALID/ GEP_V(3000,200,12), ET_V(3000,200,12)&
          ,GPP_V(3000,200),NPP_V(3000,200), FLOW_V(200,12),FLOW(200,12)&
          ,RUNOFF_V(3000,200),RUN_OFF(3000,200), &
          BASEFLOW_V(200,12),BASEFLOW(3000,200,12)

! CELLINFO
      REAL LADUSE,LATUDE,LONGI,HUCELE
      INTEGER HUCNO
      COMMON/CELLINFO/LADUSE(3000),HUCNO(3000),LATUDE(3000),LONGI(3000),HUCELE(3000)

! VEGINFO
      INTEGER veg
      COMMON/VEGINFO/VEG(3000,50)
...
! OUTPUT1
      ...
      COMMON/OUTPUT1/ PET(200,12),APET(12),PAET(200,12),APAET(12),&
         ...
         A_ET(3000,200, 12),P_ET(3000,200, 12),Sun_ET(3000,200, 12),&
         RUN_HRU(3000,200, 12),BASE_HRU(3000,200, 12)      
      
! CLIMATE      
      REAL  RAIN,TEMP,AAPPT
      COMMON/CLIMATE/ RAIN(3000,200,12), TEMP(3000,200, 12), AAPPT(3000)
      
! LAI        
      REAL LAI
      COMMON/LAI/LAI(3000,200,12)
...
! SOIL
      REAL LZTWM, LZFPM, LZFSM,LZSK,LZPK, UZTWM, UZFWM, UZK, ZPERC,&
        REXP, PFREE, SMC        
      COMMON/SOIL/LZTWM(3000), LZFPM(3000), LZFSM(3000), LZSK(3000),&
        LZPK(3000), UZTWM(3000), UZFWM(3000), UZK(3000), ZPERC(3000),&
        REXP(3000), PFREE(3000), SMC(12)
...
! FLOW 
      ...
      COMMON/FLOW/STRFLOW(3000, 200, 12),STRET(3000, 200, 12)&
     ,STRGEP(3000, 200, 12)

! CARBON
        REAL GEPM,RECOM,NEEM,GEPA,NEEA
      COMMON/CARBON/ GEPM(3000, 200, 12),RECOM(3000,200,12), &
      NEEM(3000,200,12),GEPA(3000,200),NEEA(3000,200)
      
! BYLAND     
      REAL RUNLAND,ETLAND,GEPLAND    
      COMMON/BYLAND/ RUNLAND(3000,10,12,31), &
        ETLAND(3000,10,12,31), GEPLAND(3000,10,12,31)

! HUCPETAET
      REAL HUCAET, HUCPET,HUCPAET
      COMMON/HUCPETAET/HUCAET(3000,200), HUCPET(3000,200),&
      HUCPAET(3000,200)
...
!--Open Input files----------------------------------------------
      ...
      OPEN(3,FILE='C:\WaSSICBZB\Inputs\vegINFO.TXT')
...
! ---Open Output files---------------------------------------- 
      ...
      OPEN(99,FILE='C:\WaSSICBZB\Outputs\ceshi.TXT')
